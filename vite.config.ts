...
function sociletApi(env: Record<string, string>) {
  const run = (req: unknown, res: unknown, next: () => void) => {
    ...
    if (route.startsWith("/api/ai")) {
      const spec = pathToFileURL(path.join(import.meta.dirname, "server", "ai-api.mjs")).href;
      void import(spec).then((m: { handleAiRequest: (req: unknown, res: unknown, env: Record<string, string>) => Promise<boolean> }) =>
        m.handleAiRequest(req, res, { ...process.env, ...env } as Record<string, string>),
      );
      return;
    }
    if (route.startsWith("/api/ads")) {
      const spec = pathToFileURL(path.join(import.meta.dirname, "server", "ads-api.mjs")).href;
      void import(spec).then((m: { handleAdsRequest: (req: unknown, res: unknown, env: Record<string, string>) => Promise<boolean> }) =>
        m.handleAdsRequest(req, res, { ...process.env, ...env } as Record<string, string>),
      );
      return;
    }
